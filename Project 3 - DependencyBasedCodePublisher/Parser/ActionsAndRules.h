...
{  
  ...
  */
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test using definition: ", *pTc);
  ...
  class HandleUsingDefinition : public IAction
  {
	  Repository* p_Repos;

  public:
	  HandleUsingDefinition(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Handle using definition: ", *pTc);
		  ...
		  GrammarHelper::showParse("using def", *pTc);
	  }
  };
}
#endif
